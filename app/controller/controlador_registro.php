...
if (!empty($_POST["btnregistrar"])) {
    if (!empty($_POST["nombre"]) && !empty($_POST["correo"]) && !empty($_POST["password"]) && !empty($_POST["confirm_password"])) {
        ...
        if ($stmtCheck->num_rows > 0) {
            ...
            $stmt = $conexion->prepare("INSERT INTO usuarios (tipo_usuario, nombre, cedula, rut, correo, password) VALUES (?, ?, ?, ?, ?, ?)");
            $stmt->bind_param("ssssss", $tipo_usuario, $nombre, $cedula, $rut, $correo, $password_hash);
            
            if ($stmt->execute()) {
                echo "<div class='alert alert-success' style='color: green;'>¡Registro exitoso! Redirigiendo...</div>";
                echo "<script>
                        setTimeout(function() {
                            window.location.href = '/voltmap/app/view/content/login.php';
                        }, 2000);
                      </script>";
            } else {
